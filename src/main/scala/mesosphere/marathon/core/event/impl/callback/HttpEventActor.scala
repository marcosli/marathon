...
object HttpEventActor {
  ...
  class HttpEventActorMetrics {
    ...
    val outstandingCallbacks: MinMaxCounter = Kamon.metrics.minMaxCounter(metrics.name(pre, clazz, "outstanding-callbacks"))
    // the number of events that are broadcast
    val eventMeter: MinMaxCounter = Kamon.metrics.minMaxCounter(metrics.name(pre, clazz, "events"))
    // the number of events that are not send to callback listeners due to backoff
    val skippedCallbacks: MinMaxCounter = Kamon.metrics.minMaxCounter(metrics.name(pre, clazz, "skipped-callbacks"))
    // the number of callbacks that have failed during delivery
    val failedCallbacks: MinMaxCounter = Kamon.metrics.minMaxCounter(metrics.name(pre, clazz, "failed-callbacks"))
    // the response time of the callback listeners
    val callbackResponseTime: Timer = Timer(metrics.name(pre, clazz, "callback-response-time"))
  }
}
